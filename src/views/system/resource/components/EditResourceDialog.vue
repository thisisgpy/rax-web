...
// 表单数据
const formData = reactive<UpdateResourceDto>({
  id: 0,
  code: '',
  name: '',
  type: 0,
...
watch(() => props.visible, (newValue) => {
  if (newValue && props.resourceData) {
    Object.assign(formData, {
      id: props.resourceData.id,
      code: props.resourceData.code,
      name: props.resourceData.name,
      type: props.resourceData.type,
      parentId: props.resourceData.parentId,
      path: props.resourceData.path || '',
      component: props.resourceData.component || '',
      icon: props.resourceData.icon || '',
      sort: props.resourceData.sort,
      isHidden: props.resourceData.isHidden,
      isKeepAlive: props.resourceData.isKeepAlive,
      isExternalLink: props.resourceData.isExternalLink
    })
  }
})
...
const handleSave = async () => {
  ...
  try {
    ...
    if (error !== 'validation-failed') {
      ...
      ElMessage.error(error.message || '更新资源失败，请重试')
    }
  } finally {
    saving.value = false
  }
